...
import { bulkTaskCreate } from 'src/store/apps/projects'
...
const LeaveFile = ({ isOpenImport, setOpenImport }) => {
    ...
    const handleConvertToBase64 = async () => {
        if (files.length > 0) {
            try {
                ...
                setOpenImport(false)
                debugger
                dispatch(BulkLeaveRequest({ base64string: base64 })).then(res => {
                    handleRes(res)
                    setLoading(false)
                });
            } catch (error) {
            ...
            <Dialog fullWidth open={isOpenImport} maxWidth='sm' onClose={handleClose}>
                ...
                >
                    ...
                    <Box sx={{ mb: 8, textAlign: 'center' }}>
                        <Typography variant='h5'>Add Leave Sheet</Typography>
                        <Typography variant='body2'>Add Leave sheet for your request</Typography>
                    </Box>
