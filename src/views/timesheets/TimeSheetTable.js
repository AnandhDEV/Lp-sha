...
const TimeSheetTable = ({ popperPlacement, loading, timeSheets, disable }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      headerName: 'Hours',
      field: 'burnedHours',
      renderCell: params => { `${params.value?.split(':')[0]}:${params.value?.split(':')[1]}` }
    },
  ...
  const processRowUpdate = async newRow => {
    ...
    if (hours == '00:00') {
      setLoading(false)
      customErrorToast('Burned hours is invalid')
